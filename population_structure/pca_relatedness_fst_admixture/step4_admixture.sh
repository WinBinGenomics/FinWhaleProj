...
WORKDIR="${HOMEDIR}/fin_genomics/PopStructure/${DATASET}/${REF}/DemoPassInds"
...
for MAFCUT in ${MAFLISTS[@]}; do
    ...
    cd ${OUTDIR}
    # filenames
    #OUT=JointCalls_${DATASET}_biallelic_all_LDPruned_maf${MAFCUT}_SA_mrf # all 70 dataset
    #OUT=Low_genoRemove_LDPruned_maf${MAFCUT}_SNPs # LowGenoRm Dataset
    OUT=DemoPassInds_LDPruned_maf${MAFCUT}_SNPs # For Demographic Analysis Passing Individuals Dataset
    #SHORTOUT=${DATASET}_pass_maf${MAFCUT}
    SHORTOUT=DemoPassInds_pass_maf${MAFCUT} # For Demographic Analysis Passing Individuals Dataset
    PED=${WORKDIR}/${OUT}.ped
    echo -e "K,iter,CVERROR,LL" > Admixture_CV_DemoPassInds_LLsummary_maf${MAFCUT}.csv
    for K in {2..6};do
        for i in {1..10};do
            ...
            echo -e "${K},${i},${CVERROR},${LL}" >> Admixture_CV_DemoPassInds_LLsummary_maf${MAFCUT}.csv
        done
    done
