...
workdir = paste('/data/shared/snigenda/finwhale_projects/fin_genomics/PopStructure/ESP_admixture', sep = '/')
setwd(workdir)
indir = paste0('./Admixture_20250807/maf05/') # adjust path as needed
outdir = './derive_data/'
plotdir = './plots/'
...
popmap = read.csv(file = "/data/shared/snigenda/finwhale_projects/scripts/winfingenomics/config/ESP_popmap.csv", stringsAsFactors = F) # For all70 -> all70_fin_popmap.csv | LowGeno Data -> LowGenoRm_fin_popmap.csv | For DemoPass -> DemoPassInds_fin_popmap.csv


plinkname = read.table(file = '/data/shared/snigenda/finwhale_projects/fin_genomics/PopStructure/ESP_admixture/JointCalls_ESPOnly_biallelic_all_LDPruned_maf05_SA_mrf.nosex', header = FALSE, stringsAsFactors = FALSE)
...
adprefix = paste0(indir, 'ESPOnly_biallelic_all_LDPruned')
...
# load CV data ========
adcv = read.csv(file = paste0(indir, 'Admixture_CV_ESPOnly_LLsummary_maf05.csv'), stringsAsFactors = FALSE)
...
ggsave(filename = '10iterations_ESP_Only_AdmixtureQ_all70_maf05.pdf', path = plotdir, plot = pp, height = 8, width = 12)
