...
WORKDIR="/data/shared/snigenda/finwhale_projects/fin_genomics/PopStructure/ESP_admixture"
VCFFILEDIR="/data/shared/snigenda/finwhale_projects/fin_genomics/PopStructure/ESP_admixture"
    ...
    for VCF in "$VCFFILEDIR"/*.vcf; do
        if [[ -f "$VCF" ]]; then
            ...
            if [[ "$VCF" == *"maf${MAFCUT}"* ]]; then
                OUT="${WORKDIR}/JointCalls_ESPOnly_biallelic_all_LDPruned_maf${MAFCUT}_SA_mrf"
