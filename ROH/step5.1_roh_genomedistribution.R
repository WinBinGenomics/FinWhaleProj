...
rohcatbrewer <- brewer.pal(name = "Reds", n = 9)[c(5, 7, 9)]
...
pp2 <- ggplot(zooroh) +
  ...
  theme(
    legend.position = "top",
    # Rotate x-axis TICK LABELS to be horizontal (angle = 0) and place right below ticks
    axis.text.x = element_text(
      angle = 0,
      vjust = 1,        # push label right under the tick
      hjust = 0.5,      # center label under tick
      margin = margin(t = 5)  # add a small top margin to separate from axis line
    ),
...
ggsave(
  filename = "FigureS8.genomewide_distribution_RZooRoH_newcat_20210912.pdf",
  plot = pp,
  path = plotdir,
  width = 12,
  height = 12
