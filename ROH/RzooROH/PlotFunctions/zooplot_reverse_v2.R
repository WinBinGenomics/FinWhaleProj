...
zooplot_partitioning_v1 <- function (input, cols=NULL, plotids=TRUE, toplot=NULL,
  ...
  if (length (input) >1) {
    ...
    # how many HBD‐classes (krates) are there?
    n_krates <- length(unique(allres3$krates))

    # how many samples per population?
    sample_counts <- sapply(input, function(zres) length(zres@sampleids))

    # now repeat each pop name by (samples × krates)
    allres3$Groups <- rep(names(input), times = sample_counts * n_krates)
