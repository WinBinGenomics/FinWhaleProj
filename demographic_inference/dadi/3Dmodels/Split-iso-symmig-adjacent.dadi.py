...
#modelName="3D.Split"
modelName=Split-iso-symmig-adjacent
...
def refugia_adj_3(params, ns, pts):
    ...
    T1a: Pop1 vs PopA isolation (no migration)
    T1b: The scaled time between the split of pops 1 vs 2 and 3 (in units of 2*Na generations).
...
T1a_scaled_gen = popt[7] * 2 * Nanc   # early isolation time
T1b_scaled_gen = popt[8] * 2 * Nanc   # secondary contact time
T2_scaled_gen  = popt[9] * 2 * Nanc   # split between pop2 and pop3
...
scaled_param_names = ("Nanc", "N1", "NA_23", "N2", "N3", "T1a_gen", "T1b_gen", "T2_gen", "mA_pergen", "m1_pergen", "m2_pergen")

scaled_popt = (Nanc, nu1_scaled_dip, nuA_scaled_dip, nu2_scaled_dip, nu3_scaled_dip, T1a_scaled_gen, T1b_scaled_gen, T2_scaled_gen, mA_pergen, m1_pergen, m2_pergen)
