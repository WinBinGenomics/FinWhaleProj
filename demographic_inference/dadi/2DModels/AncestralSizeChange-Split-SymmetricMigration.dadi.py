...
modelName="AncestralSizeChange-Split-SymmetricMigration"
...
def sym_mig_size(params, ns, pts):
    ...
    phi = Integration.two_pops(phi, xx, Td, nu1=nu1, nu2=nu2,m12=m, m21=m) #Divergence time
...
param_names=("Ta","nua","Td","nu1","nu2","m")

lower_bound = [1e-4, 1e-9, 1e-5, 1e-5, 1e-5, 1e-9]
upper_bound = [10, 30, 10, 10, 10, 20] # 20 as upper bound on mig rec by dadi
p0 = [0.2, 2.0 ,0.10, 2.0, 2.0, 2.0] # initial parameters

func=sym_mig_size # set the function
...
m_fraction = popt[5] / (2 * Nanc)

scaled_param_names = ("Nanc","Ta","nua","Td","nu1","nu2","migrationFraction")
scaled_popt = (Nanc, Ta_scaled_gen, nua_scaled_dip, Td_scaled_gen,
               nu1_scaled_dip, nu2_scaled_dip, m_fraction)
