...
modelName="AncestralSizeChange-Split-Isolation-AsymmetricMigration"
...
def asym_mig_size(params, ns, pts):
    ...
    phi = Integration.two_pops(phi, xx, Td, nu1=nu1, nu2=nu2,m12=0, m21=0) #Isolation
    
    phi = Integration.two_pops(phi, xx, Tc, nu1=nu1, nu2=nu2,m12=m12, m21=m21) #Asymmetric Migration
...
param_names=("Ta","nua","Td","nu1","nu2","Tc","m12","m21")

lower_bound = [1e-4,1e-9,1e-5,1e-5,1e-5,1e-5,1e-9,1e-5]
upper_bound = [10,20,10,10,10,10,20,20] # 20 as upper bound on mig rec by dadi
p0 = [0.2,2.0,0.10,2.0,2.0,0.01,2.0,2.0] # initial parameters
...
m12_fraction=popt[6]/(2*Nanc)
m21_fraction=popt[7]/(2*Nanc)


scaled_param_names=("Nanc","Ta","nua","Td","nu1", "nu2","Tc", "migrationFraction12", "migrationFraction21")

scaled_popt=(Nanc,Ta_scaled_gen,nua_scaled_dip,Td_scaled_gen,nu1_scaled_dip,nu2_scaled_dip,Tc_scaled_gen,m12_fraction,m21_fraction)
