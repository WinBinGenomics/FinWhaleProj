...
modelName="Split-AsymmetricMigration"
...
def split_WAmig(params, ns, pts): 
    ...
    phi = Integration.two_pops(phi, xx, TDiv, nu1, nu2, m12=m12, m21=m21)  # two pops at diff sizes with asymmetric migration
    fs = Spectrum.from_phi(phi, ns, (xx,xx)) 
    
    return fs

#param_names=("nu1","nu2", "m12", "m21", "TDiv")
#lower_bound = [1e-2, 1e-4, 1e-4, 1e-4, 1e-5]
#upper_bound = [10, 20, 20, 20, 10] # 20 as upper bound on mig rec by dadi
#p0 = [1.31,0.0071,2.3,10,0.15] # initial parameters; changing starting mu parameter because won't get permuted

param_names = ("nu1", "nu2", "m12", "m21", "TDiv")
lower_bound = [0.75, 0.75, 0.0, 1e-4, 1e-4]
upper_bound = [5, 10, 20.0, 20.0, 0.30]

# Midpoint starting values
p0 = [2.0, 2.0, 2.0, 2.0, 0.18]
...
T_scaled_gen=popt[4]*2*Nanc
m12_fraction=popt[2]/(2*Nanc)
m21_fraction=popt[3]/(2*Nanc)

# THIS IS TRICKY: confirmed here: https://groups.google.com/forum/#!topic/dadi-user/AY_4NoEjsx8

scaled_param_names=("Nanc","nu1","nu2","migrationFraction12", "migrationFraction21","T")
scaled_popt=(Nanc,nu1_scaled_dip,nu2_scaled_dip,m12_fraction,m21_fraction,T_scaled_gen)
