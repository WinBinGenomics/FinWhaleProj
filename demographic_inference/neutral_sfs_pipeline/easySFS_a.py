...
def read_input(vcf_name, all_snps=False, verbose=False):
	...
	if vcf_name.endswith(".gz"):
		ofunc = lambda f: gzip.open(f, 'rt')
	else:
		ofunc = lambda f: open(f, 'r')
	with ofunc(vcf_name) as infile:
		for line in infile:
			if line.startswith("#CHROM"):
				header = line.strip().split('\t')
				break
	if not header:
		raise Exception("VCF header not found")
...
def read_inputOLD(vcf_name, all_snps=False, verbose=False):
