...
def count_PassingMonomorphicSites(pops,projDict,VCF):
    ...
    pairDict=dict()
    # set up 3D dictionary for monomorphic sites that pass for all three populations. added by KW 10/11/25
    tripleDict=dict()
    # combos of every pair and every triples of populations (for 2D and 3D - KW 10/11/25)
    combos=list(combinations(pops.keys(),2)) # every pair of populations
    triples=list(combinations(pops.keys(),3)) # for 3D
    ...
    for popPair in combos:
        pairDict[popPair]=0
    for popTriple in triples: # added by KW for 3D  
    	tripleDict[popTriple]=0
    # skip header lines
    for line0 in inVCF:
        if line0.startswith('#'):
            continue
        ...
        if "0/1" in allCalls or "1/0" in allCalls or "1|0" in allCalls or "0|1" in allCalls or "1|1" in allCalls or "1/1" in allCalls:
            ...
            for popPair in combos:
                ...
                if passDict[pop1]=="PASS" and passDict[pop2]=="PASS":
                    ...
                    continue
          # Get sites that pass projection values for each triple of populations. added by kw for 3D
            for popTriple in triples:
                pop1 = popTriple[0]
                pop2 = popTriple[1]
                pop3 = popTriple[2]
                # site must pass all three projections
                if (passDict[pop1] == "PASS" and
                    passDict[pop2] == "PASS" and
                    passDict[pop3] == "PASS"):
                    tripleDict[popTriple] += 1
                else:
                    continue
                
    #print(str(TotalDict))
    inVCF.close()
    return countDict, pairDict, tripleDict
    
singlePopcounts, popPaircounts, popTriplecounts = count_PassingMonomorphicSites(pops,projDict,vcfFile)
...
# write out triple pop counts (for 3D SFS)
triplePop_outputFile = open(str(outdir) + "/countsOfMonomorphicPassingProjectionThresholds.perTriple." + str(prefix) + ".txt", "w")
triplePop_outputFile.write("population1\tpopulation2\tpopulation3\tHomREFcountPassingAllThreeProjThresholds\tProjectionValue1\tProjectionValue2\tProjectionValue3\n")

for key, value in popTriplecounts.items():
    # key is a tuple: (pop1, pop2, pop3)
    triplePop_outputFile.write('{0}\t{1}\t{2}\t{3}\t'.format(key[0], key[1], key[2], value))
    triplePop_outputFile.write(str(projDict[key[0]]) + "\t")
    triplePop_outputFile.write(str(projDict[key[1]]) + "\t")
    triplePop_outputFile.write(str(projDict[key[2]]) + "\n")

triplePop_outputFile.close()
